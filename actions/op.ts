...
export const getBasicSearchResults = async (search_result_id: string): Promise<SearchResultType[]> => {
    ...
    const pdf_store_ids = pdf_texts.map(item => item.pdf_store_id);
    ...
    const data = pdf_texts.map(({ pdf_store_id, pages }) => {
        ...
        return {
            ...
            relevantPages: [], // Empty array as relevance is not calculated yet
        };
...
export const calculateRelevance = async (search_result_id: string, pdf_texts: Array<{pdf_store_id: string, pages: number[]}>): Promise<SearchResultType[]> => {
    const ts = Date.now();
    const db = await DB.getInstance();
...
// Combined function that maintains the original behavior
export const handleQuery = async (search_result_id: string): Promise<SearchResultType[]> => {
    ...
    const difTs = Date.now() - ts;
    console.log("Total time elapsed in secs: ", difTs / 1000);
