...
export const getBasicSearchResults = async (
	...
	const pdf_files_promises = pdf_urls.map(url => downloadLimit(() => downloadPdf(url)));

	const pdf_files = (await Promise.all(pdf_files_promises)).filter(({ is_error }) => !is_error);
	...
	const pdf_stores = (await Promise.all(pdf_store_promises)).filter(Boolean) as Array<{
		pdf_store_id: string;
		blob: Blob;
		url: string;
	}>;
	...
	const pdf_store_map = new Map(pdf_store_records.map(store => [store.id, store]));

	// Map the results efficiently using the lookup maps
	const data = pdf_stores
		.map(({ pdf_store_id }) => {
...
export const calculateRelevance = async (
	search_result_id: string,
	pdf_stores: Array<{ pdf_store_id: string }>,
): Promise<SearchResultType[]> => {
	const ts = Date.now();
	const db = await DB.getInstance();
	...
	const pdf_store_records = await db
		...
		.where(
			"id",
			"in",
			pdf_stores.map(item => item.pdf_store_id),
		)
		.execute();
	...
	const downloaded_pdfs = (await Promise.all(download_promises)).filter(
		({ is_error }) => !is_error,
	);
	...
	const pdf_texts = (await Promise.all(process_promises)).filter(
	...
	const relevant_pdfs = (await Promise.all(relevance_promises)).filter(
	...
	// Create a map for faster lookups
	const pdf_parsed_map = new Map(pdf_parsed_records.map(record => [record.id, record]));

	// Get all unique pdf_store_ids
	const store_ids = Array.from(new Set(pdf_parsed_records.map(record => record.pdf_store_id)));
