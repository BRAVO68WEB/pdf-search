...
export async function GET(request: Request) {
  ...
  try {
    ...
    const parsedPdfs = await db.selectFrom("pdf_parsed")
      .selectAll()
    ...
    const storedPdfs = await db.selectFrom("pdf_stores")
        .selectAll()
        .where("id", "in", parsedPdfs.map((pdf) => pdf.pdf_store_id))
        .execute();
